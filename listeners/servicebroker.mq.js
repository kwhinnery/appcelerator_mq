    ...
    try {
      ...
      App.mq.api.ajax({
        ...
        success: function(data){
          App.mq.sbconfig.url = data.getElementsByTagName('servicebroker')[0].childNodes[0].nodeValue.replace(/@{rootPath}/,"/");
  ...
  $MQL(/r:.*|remote:.*/, function(msg) {
    ...
    try {    
      ...
      App.mq.api.ajax({
        ...
        data: App.mq.JSON.stringify(json),
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
        },
        success: function(data) {
          data = App.mq.JSON.parse(data);
          var msgPrefix = (msg.name.indexOf("remote:")!=-1) ? "remote:" : "r:";
          $MQ(msgPrefix+data.messages[0].type, data.messages[0].data, {
            scope: data.messages[0].scope,
            appcRemoteResponse: true
          });
        },
        error: function(xhr) {
          $MQ("service.invocation.error", xhr.responseText);
        }
      });
    } catch(e) {
      $MQ("service.broker.listener.error",e);
    }
